  ...
  _createClass(CSSDataURI, [{
    ...
    value: function _encodeAssets(data) {
      ...
      matches.forEach(function (match) {
        ...
        if (!assets[url] && (0, _multimatch2.default)([file], _this2.options.filter).length) {
          assets[url] = {
            file: file
          };
        }
    ...
    value: function encode(src, dest) {
      ...
      _fs2.default.readFile(src, "utf8", function (err, data) {
        if (err) {
          _this3._handleComplete(err, null, callback);
          return;
        }
        _this3._encodeData(data, src, function (err, content) {
          if (err) {
            _this3._handleComplete(err, null, callback);
            return;
          }
          ...
          _fs2.default.writeFile(dest, content, "utf-8", function (err) {
            _this3._handleComplete(err, content, callback);
          });
        });
      });
      return this;
    ...
    value: function promise(src, dest, options) {
      var instance = new CSSDataURI(options);
      return new Promise(function (resolve, reject) {
        instance.on('complete', resolve).encode(src, dest).on('error', reject).on('success', resolve);
      });
    }
  }, {
    key: 'sync',
